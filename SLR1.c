...
void SLR_driver(state_stack *state,char_stack *input,char action1[12][6],
          int action2[12][6],int SLR_goto[12][3], production p[7] )
{
  int s,k,l,m;
  char c,ch;
  s=read_state(state);
  ...
  {  
     ...
     case 'r': m=action2[s][k];
               for(l=p[m].length;l>0;l--)
                 state_pop(state);
               state_push(state,SLR_goto[read(state)][vn_to_int(p[m].left)]);
               break;
     case 'a': printf("\naccept!"); getch(); exit(0);
     default: error();
     }
...
main()
{
  ...
  int action2[12][6]={
      ...
      {2,-1,-1,-1,9,-1}};
  int SLR_goto[12][3]={
      {1,6,3},{-1,-1,-1},{-1,10,3},{-1,-1,-1},{-1,-1,-1},{11,6,3},
